...
class MyFrame(wx.Frame):
    ...
    def visitall( self, obj, indent ):
        indent += '\t'
        if hasattr( obj, 'GetChildren' ) is True:
            for x in obj.GetChildren():
                #print indent, '#', type(x)
                self.visitall( x, indent )

        if hasattr( obj, 'GetLabel' ) is True:
            #print indent, type(obj) , 'LABEL:' + obj.GetLabel()
            pass
    ...
    def load_settings(self):
        if os.path.isfile('atop.settings'):
            with open('atop.p','rb') as f:
    ...
    def save_settings_handler(self, event):  # wxGlade: MyFrame.<event_handler>
        ...
        self.visitall( self , '')
        pprint.pprint(self.settings)
        with open( 'atop.settings', 'w') as f:
